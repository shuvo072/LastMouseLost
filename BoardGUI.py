            ...
            for i in range(len(self.board)):
                for j in range(len(self.board[0])):
                    if self.board[i][j] != newBoard[i][j]:
                        ...
                        if newBoard[i][j] < 0:
                            self.tiles[i][j] = self.c.create_oval(j*self.col_width+14, i*self.row_height+14,
                                                              (j+1)*self.col_width-14, (i+1)*self.row_height-14,
                                                              fill="black")
                        elif newBoard[i][j] > 0:
                            self.tiles[i][j] = self.c.create_oval(j*self.col_width+14, i*self.row_height+14,
                                                                  (j+1)*self.col_width-14, (i+1)*self.row_height-14,
    ...
    def processClick(self, event):
        ...
        if not self.checkerSelected:
            ...
            if self.board[row][col] != 0 and self.isCurrentPlayerChecker(row, col):
                ...
                self.tiles[row][col] = self.c.create_oval(col*self.col_width+14, row*self.row_height+14,
                                                         (col+1)*self.col_width-14, (row+1)*self.row_height-14,
                                                          fill="green")
           ...
           # print(row, col)
            ...
            self.tiles[oldrow][oldcol] = self.c.create_oval(oldcol*self.col_width+14, oldrow*self.row_height+14,
                                                            (oldcol+1)*self.col_width-14, (oldrow+1)*self.row_height-14,
                                                            fill="gray")
